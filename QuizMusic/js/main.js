...
casilla1 = document.getElementById("casilla1");
casilla2 = document.getElementById("casilla2");
casilla3 = document.getElementById("casilla3");
casilla4 = document.getElementById("casilla4");
thirdChance = document.getElementById("thirdChance");
secondChance = document.getElementById("secondChance");
lastChance = document.getElementById("lastChance");
interfaz = document.getElementById("interfaz");
barIns = document.getElementById("barIns");
timerNumb = document.getElementById("timerNumb");
bar = document.getElementById("bar");
logoCategorias = document.getElementById("logoCategorias");
...
function toSection(sect){
    ...
    time = 10;
    gains = 25;
    thirdChance.src = "img/Estrellas/Asset 108.png"
    secondChance.src = "img/Estrellas/Asset 108.png"
    lastChance.src = "img/Estrellas/Asset 108.png"
}
...
function respuestas() {
    if (indexArray === 5) { //YA HABIENDO LLEGADO AL FINAL, HAY QUE RESETEAR EL INDEX. 
        ...
        total = parseInt(score0.innerHTML) + 
        parseInt(score1.innerHTML) + 
        parseInt(score2.innerHTML) + 
        parseInt(score3.innerHTML);
    ...
    casilla1.classList.remove("true");
    casilla2.classList.remove("true");
    casilla3.classList.remove("true");
    casilla4.classList.remove("true");
    casilla1.classList.remove("false");
    casilla2.classList.remove("false");
    casilla3.classList.remove("false");
    casilla4.classList.remove("false");

    time = 10;
    gains = 25;
    tiempoActual = 10;
    timerNumb.innerHTML = tiempoActual;
    update = setInterval(timing, 1000);
    bar.src = bars[indexArray+1];
    logoCategorias.src = categorias[game].pregunta[indexArray].opcion[4];
    ...
    casilla1.innerHTML = casillas[0];
    casilla2.innerHTML = casillas[1];
    casilla3.innerHTML = casillas[2];
    casilla4.innerHTML = casillas[3];
}
...
function verify(choice) {
    chosenOpt = document.getElementById(choice);
    animacion.className = ("animated zoomIn");
    if(chosenOpt.innerHTML == categorias[game].pregunta[indexArray].opcion[0]) {
        score = score + gains;
        logoCategorias.src = categorias[game].pregunta[indexArray].opcion[5];
        indexArray ++;
        chosenOpt.classList.add("true");
        clearInterval(update);
        setTimeout(respuestas, 1000);
    }else {
        chosenOpt.classList.add("false");
        if(lives === 3) {
            thirdChance.src = "img/Estrellas/Asset 109.png"
            lives --;
        }else
        if (lives === 2) {
            secondChance.src = "img/Estrellas/Asset 109.png"
            lives--;
        }else {
            lastChance.src = "img/Estrellas/Asset 109.png"
            lives--;
            indexArray = 0;
            lives = 3;
            clearInterval(update);
...
function timing() {
    ...
    if (tiempoActual===0) {
        clearInterval(update);
        indexArray++;
        casilla1.classList.add("false");
        casilla2.classList.add("false");
        casilla3.classList.add("false");
        casilla4.classList.add("false");
        setTimeout(respuestas, 1000);
    }
}
